...
def _multinomial_sample(weights: Tensor, n: int, replacement: bool = True) -> Tensor:
    ...
    debug = os.getenv("GSPLAT_MCMC_DEBUG", "").strip().lower() in {"1", "true", "yes", "on"}
    weight_sum = float(weights.sum().item())
    if (not torch.isfinite(weights).all()) or weight_sum <= 0.0:
        if debug:
            finite_ratio = float(torch.isfinite(weights).float().mean().item()) if weights.numel() > 0 else 0.0
            print(
                f"[mcmc-debug] multinomial fallback: n={n} num={num_elements} "
                f"sum={weight_sum:.6e} min={float(torch.nan_to_num(weights, nan=0.0).min().item()):.6e} "
                f"max={float(torch.nan_to_num(weights, nan=0.0).max().item()):.6e} finite_ratio={finite_ratio:.3f}",
                flush=True,
            )
        # Fall back to a uniform distribution when the proposal distribution collapses.
...
def relocate(
    ...
    use_alive_indices = True
    if probs.numel() == 0 or (not torch.isfinite(probs).all()) or float(probs.sum().item()) <= 0.0:
        if os.getenv("GSPLAT_MCMC_DEBUG", "").strip().lower() in {"1", "true", "yes", "on"}:
            print(
                f"[mcmc-debug] relocate fallback: dead={int(dead_indices.numel())} alive={int(alive_indices.numel())} "
                f"opacity_min={float(opacities.min().item()):.6e} opacity_max={float(opacities.max().item()):.6e} "
                f"opacity_mean={float(opacities.mean().item()):.6e}",
                flush=True,
            )
        probs = opacities.flatten()
        use_alive_indices = False
    if probs.numel() == 0 or (not torch.isfinite(probs).all()) or float(probs.sum().item()) <= 0.0:
        probs = torch.ones((len(opacities),), device=opacities.device, dtype=opacities.dtype)
        use_alive_indices = False
    sampled_idxs = _multinomial_sample(probs, n, replacement=True)
    if use_alive_indices and alive_indices.numel() > 0:
...
def sample_add(
    ...
    if probs.numel() == 0 or (not torch.isfinite(probs).all()) or float(probs.sum().item()) <= 0.0:
        if os.getenv("GSPLAT_MCMC_DEBUG", "").strip().lower() in {"1", "true", "yes", "on"}:
            print(
                f"[mcmc-debug] sample_add fallback: n={n} opacity_min={float(opacities.min().item()):.6e} "
                f"opacity_max={float(opacities.max().item()):.6e} opacity_mean={float(opacities.mean().item()):.6e}",
                flush=True,
            )
        probs = torch.ones((len(opacities),), device=opacities.device, dtype=opacities.dtype)
    sampled_idxs = _multinomial_sample(probs, n, replacement=True)
